...
function callExpression(callee, args) {
    return core.types.callExpression(callee, args);
}
function arrowFunctionExpression(params, body) {
    return core.types.arrowFunctionExpression(params, body);
}
function stringLiteral(value) {
    return core.types.stringLiteral(value);
}
function objectProperty(key, value) {
    return core.types.objectProperty(key, value);
}
function objectExpression(properties) {
    return core.types.objectExpression(properties);
}
function memberExpression(object, property, computed = false) {
    return core.types.memberExpression(object, property, computed);
}
function variableDeclaration(identifier, ast) {
    return core.types.variableDeclaration('const', [
        core.types.variableDeclarator(typeof identifier === 'string' ? core.types.identifier(identifier) : identifier, ast)
    ]);
}
...
function isTopLevelCalled(path) {
    return core.types.isProgram(path.parent) || core.types.isExportDefaultDeclaration(path.parent) || core.types.isExportNamedDeclaration(path.parent);
}
...
function ensureCSSValueASTKind(value) {
    ...
    switch(typeof value){
        case 'undefined':
            ast = core.types.identifier('undefined');
            break;
        case 'string':
            if (value === 'undefined') {
                ast = core.types.identifier('undefined');
            } else if (MARK.isReference(value)) {
                ast = core.types.identifier(value.slice(2));
            } else {
                ast = core.types.stringLiteral(value);
            }
            break;
        case 'object':
            ast = core.types.nullLiteral();
            break;
        case 'number':
            ast = core.types.numericLiteral(value);
    }
    return ast;
}
    ...
    toJSAST() {
        ...
        for(let i = 0; i < mergedCSSRules.length; i++){
            ...
            if (vairableNames.size) {
                ...
                if (condit) {
                    expressionAST.push(core.types.logicalExpression('&&', condit.node, variables.length ? callee : expression));
                } else {
                    expressionAST.push(callee);
                }
                if (variables.length) {
                    const fnAST = arrowFunctionExpression(variables.map((s)=>core.types.identifier(MARK.isReference(s) ? s.slice(2) : s)), jsAST);
...
function getCSSVarName(themeName, key, classNamePrefix) {
    return `var(--${classNamePrefix + shared.utils.hash(`${themeName}.${key}`)})`;
}
    ...
    evaluateCSSVariableFromStylex(s, jsAST) {
        ...
        const getCssValue = (belong, attr)=>{
            if (this.ctx.fileNamesForHashing.has(belong)) {
                ...
                const themeName = shared.utils.genFileBasedIdentifier({
                    fileName,
                    exportName
                });
...
function transformInjectGlobalStyle(path, ctx) {
    ...
    if (result) {
        ...
        const CSS = stylis.serialize(stylis.compile(sb.css), stylis.stringify);
        path.replaceWith(core.types.stringLiteral(''));
        return CSS;
    }
}
    ...
    handleImportStmt(stmts, (path)=>{
        ...
        if (matchers(importSource)) {
            ...
            for (const spec of path.node.specifiers){
                ...
                if (core.types.isImportSpecifier(spec)) {
...
function transformStylexAttrs(path, ctx) {
    ...
    if (result) {
        ...
        ctx.stmts.push(stylexDeclaration);
        path.replaceWith(core.types.jsxSpreadAttribute(callExpression(attach, expr)));
    }
}
            ...
            Program: {
                ...
                exit (path) {
                    ...
                    if (ctx.stmts.length) {
                        ...
                        const importSpecs = Object.values(identifiers).map((a, i)=>core.types.importSpecifier(a, core.types.identifier(modules[i])));
                        const importStmt = core.types.importDeclaration(importSpecs, core.types.stringLiteral('@stylexjs/stylex'));
