...
def build_prototypes(annotated_dir, classes_in_ckpt, backbone, max_per_class=200):
...
def nms_boxes(boxes, scores, iou_thresh=0.55):
...
class SimpleTracker:
    def __init__(self, max_age=15, iou_thresh=0.3, sim_thresh=0.85, lambda_sim=0.5):
        self.next_id = 0
        self.tracks = {}
    ...
    def update(self, dets):
        ...
        for tid, track in self.tracks.items():
            old = track["smooth_box"]
            new = track["box"]

            bx1 = int(0.7 * old[0] + 0.3 * new[0])
            by1 = int(0.7 * old[1] + 0.3 * new[1])
            bx2 = int(0.7 * old[2] + 0.3 * new[2])
            by2 = int(0.7 * old[3] + 0.3 * new[3])

            track["smooth_box"] = [bx1, by1, bx2, by2]
...
class YOLO_SAM2_Detector:
    def __init__(self, yolo_ckpt, sam2_ckpt, sam2_cfg):
        ...
        self.cls_color_map = {
            ...
            for name in self.yolo.model.names.values()
        }
    ...
    @torch.inference_mode()
    def infer_frame(self, frame, conf_thres=0.5, proto_sim_th=0.40):
        scale_factor = 1.0
        detections = []
        ...
        try:
            masks, scores, _ = self.predictor.predict(box=boxes_np, multimask_output=False)
        ...
        boxes_arr = np.array([d["box"] for d in detections], dtype=np.int32)
        scores_arr = np.array([d["score"] * d["sim"] for d in detections], dtype=np.float32)
        keep_idx = nms_boxes(boxes_arr, scores_arr, iou_thresh=0.25)
...
def run_webcam_hybrid():
    ...
    while True:
        ...
        dets = detector.infer_frame(frame, conf_thres=0.55, proto_sim_th=0.65)
        ...
        for cls_name in class_counts.keys():  
            if cls_name not in cls_color_map:
                cls_color_map[cls_name] = (
        ...
        for d in tracked:
            x1, y1, x2, y2 = map(int, d["smooth_box"])
            ...
            if "mask" in d:
                ...
                out = cv2.addWeighted(out, 1.0, neon, 0.35, 0)
